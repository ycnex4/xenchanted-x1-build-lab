...
PACKAGE="runtime-source-canonical-asset-id-refactor-source-only-v6-quiet"
...
def replace_count(text: str, old: str, new: str, label: str, min_count: int = 0, max_count: int | None = None) -> str:
...
def replace_exact_once(text: str, old: str, new: str, label: str) -> str:
    return replace_count(text, old, new, label, min_count=1, max_count=1)

def replace_regex(text: str, pattern: str, repl: str, label: str, min_count: int = 0, flags: int = 0) -> str:
...
# Keep test assertions against args.canonical_asset_id where existing fixtures still use the same SPL mint value;
# do not replace them with `prepared`, because not every helper/test scope has a prepared value.
write(rel, text)
...
text = replace_regex(text, r"(^\s*)mint_id\s*:", r"\1canonical_asset_id:", "processor_struct_literal_field_rename", min_count=0, flags=re.M)
...
PY
  ...
  git diff -- $CHANGED_TARGETS > "$RESULTS_DIR/canonical_asset_id_refactor_v6.diff"
...

  {
    ...
    echo "diff_path=$RESULTS_DIR/canonical_asset_id_refactor_v6.diff"
...

{
  ...
  if [ -f "$FULL_LOG" ]; then
    grep -E 'missing_pattern|unexpected_pattern_count|too_many_pattern|Traceback|SystemExit|SyntaxError|NameError|TypeError|ValueError' "$FULL_LOG" | tail -n 30 || true
  fi
  if [ -f "$LOG_DIR/cargo_test.log" ]; then
    grep -E '^(error\[|error: could not compile|warning: build failed|\s*\|.*\^|\s*= note: available fields|\s*help:)' "$LOG_DIR/cargo_test.log" | head -n 40 || true
  fi
} > "$ERROR_SUMMARY_FILE"
