...
Fresh local rebuild validated the expected hash.

The live testnet binary differs from the current local build.

This means the live testnet binary should be treated as stale, different, or unknown until a separately scoped deploy/upgrade package is prepared and approved.
...
No RPC, testnet call, ProgramData read, executable bytes dump, live hash comparison, deploy, upgrade, write-buffer, signing, submit, mutation, or activation is approved by this checkpoint.
...
Testnet deploy/upgrade package planning only after separate scope.

No deploy/upgrade execution is authorized by this document.
