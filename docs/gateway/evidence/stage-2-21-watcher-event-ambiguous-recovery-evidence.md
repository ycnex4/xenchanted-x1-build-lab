...
This document records Stage 2.21 watcher event ambiguous recovery evidence for the X1 direct mint gateway relayer prototype.
...
It adds a live regression test showing that a relayer can inspect processed_burn and recipient balance after an ambiguous watcher-event submit result instead of blindly retrying.
...
Runtime test added:
    ...
    tests/stage2_watcher_event_ambiguous_recovery.test.ts
...
- operational action is safe_retry_candidate
- reason is not_processed_after_ambiguous_result

After successful watcher-event submit:

- processed_burn exists
- balance delta equals expected minted amount
- operational action is completed_no_retry
- reason is confirmed_after_ambiguous_result

With wrong expected minted amount:

- processed_burn exists
- actual balance delta remains the real minted amount
- operational action is stop_manual_review
...
Command:
    ...
    npx ts-mocha -p ./tsconfig.json -t 120000 tests/stage2_watcher_event_ambiguous_recovery.test.ts
...
Stage 2.21 proves that ambiguous recovery works at the high-level watcher-event API boundary.

The relayer can inspect processed_burn and recipient token balance after an ambiguous watcher-event submit result and choose between completed_no_retry, safe_retry_candidate, or stop_manual_review.

This avoids blind retry behavior.
