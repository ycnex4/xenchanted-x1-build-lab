...
# Latest XXXL real PDA derivation fixture checkpoint
...
- `docs/checkpoints/xxxl-real-pda-derivation-fixture.md`
...
Status: real PDA derivation fixture only, before SPL Token CPI fixture.
...
Status: deterministic decoder/handler model only, before live runtime implementation.
...
Status: production byte layout definitions only, before live X1/SVM implementation skeleton.
...
Status: route-aware runtime skeleton only, not live X1/SVM code.
...
Status: instruction serialization vectors only, no final byte layout yet.
...
Authoritative public explanation draft:

- `docs/xxxl/xxxl-genesis-phase-public-explanation.md`

Checkpoint document:

- `docs/checkpoints/xxxl-genesis-phase-public-explanation.md`

Main public points:

- XXXL is the canonical X1-native token for the xEnchanted / X1 path.
- Genesis Phase starts gateway-only.
- XXXL is minted only through verified Ethereum XNTD gateway events.
- There is no manual mint, premine, founder allocation, or hidden emission.
...
- Final goal remains deterministic X1-native Core / Forge / Stake-like mechanics, then freeze.
...
Status: public explanation draft added, no production runtime code yet.
...
Status: docs-only planning, no production runtime code yet.
...
## Latest Stage 1 exact cryptographic test vectors checkpoint
...
Turn the Stage 1 message, encoding, hash, signature, recipient, finality, replay, amount, and authority decisions into an exact test-vector profile.

This is a design / test-vector specification milestone only.
...
Cryptographic decisions locked:
...
Gateway mint message field order locked:
...
encodedGatewayMintMessage length:
...
messageHash preimage length:

32 + 608 = 640 bytes

Sample Vector 1 locked:

- Vector ID = STAGE1_GATEWAY_VALID_001
- sourceChainId = 1
- sourceToken = 0x1111111111111111111111111111111111111111
- sourceSender = 0x2222222222222222222222222222222222222222
- sourceBurnTxHash = 0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
- sourceBurnEventIndex = 7
- sourceBlockNumber = 19000000
- sourceBlockHash = 0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
- sourceNonce = 42
- x1RecipientBytes = 0x0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20
...
- targetMintCoreId = 0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

Ed25519 signing vector requirement:

- deterministic test-only Ed25519 keys
- guardian signature payload is exactly messageHash
...
Required generated valid vectors:

- valid Stage 1 gateway mint message
- valid x1RecipientHash
- valid canonicalEventKey
- valid domainSeparator
- valid encodedGatewayMintMessage
- valid messageHash
- valid Ed25519 guardian signature over messageHash

Required invalid vectors include:
...
This milestone locks the Stage 1 exact cryptographic test vector profile:
...
## Latest Stage 1 gateway approval verifier checkpoint

The Stage 1 gateway approval verifier milestone was completed on the stage-1-gateway-approval-verifier branch.
...
- src/index.ts exports Stage 1 gateway approval verifier helpers
...
Add a single deterministic approval verification helper before guardian-set, quorum, X1 mint core, relayer, watcher, frontend gateway, or deployment runtime work begins.

The approval verifier composes:
...
- Stage1GatewayApprovalVerificationResult
- verifyStage1GatewayApproval()

The approval verifier checks both layers:
...
- guardianPublicKey length = 32 bytes
- guardianSignature length = 64 bytes
- Ed25519 signature verifies over the exact messageHash

The tests verify:

- valid message plus valid guardian signature is accepted
- invalid message is rejected even when signature is valid for the supplied hash
- invalid signature is rejected even when message is valid
- both message and signature failures are surfaced together
- malformed Ed25519 approval inputs are rejected while valid message verification remains visible
...
The approval verifier is a pure composition helper.
...
This is an approval-verifier helper milestone only.
...
Stage 2.10 adds the first relayer idempotency / retry behavior for the X1 direct mint gateway.
...
- TypeScript relayer idempotency / retry prototype
- no on-chain runtime change
- relayer checks processed_burn before submitting a transaction

Runtime changes:

- tests/helpers/stage2RelayerPrototype.ts updated
- tests/stage2_relayer_idempotency_retry.test.ts added

Confirmed behavior:

- first submission returns submitted and mints tokens
- repeated run sees processed_burn exists
- repeated run returns already_processed
- repeated run submits no new transaction
- repeated run does not mint twice

Checks passed:

- Stage 2.10 relayer idempotency live test: 1 passing
- Stage 2.9 relayer prototype live test: 1 passing
...
- docs/gateway/evidence/stage-2-10-relayer-idempotency-retry-evidence.md
...
- docs/gateway/evidence/stage-5-1-explicit-live-send-readiness-opening-boundary-evidence.md
...
The Stage 5.1 artifact is:

    stage5_explicit_live_send_readiness_opening_result

The Stage 5.1 execution mode is:

    explicit_live_send_readiness_opening_offline
...
Stage 5.1 consumes the Stage 4.20 final closure artifact:
...
Stage 5.1 defines this opening scope:
...
Future operator confirmation phrase:
...
- docs/gateway/evidence/stage-5-5-external-wallet-approval-decision-receipt-boundary-evidence.md

Stage 5.5 defines an offline receipt for a user approval or rejection decision from the external wallet layer.

The Stage 5.5 artifact is:

    stage5_external_wallet_approval_decision_receipt_result

The Stage 5.5 execution mode is:

    external_wallet_approval_decision_receipt_offline
...
- docs/gateway/evidence/stage-5-8-live-rpc-simulation-preflight-boundary-evidence.md

Stage 5.8 defines a live RPC simulation preflight boundary for a validated signed-payload quarantine reference.

The Stage 5.8 artifact is:

    stage5_live_rpc_simulation_preflight_result

The Stage 5.8 execution mode is:

    live_rpc_simulation_preflight_offline
...
The Stage 5.9 artifact is:

    stage5_external_wallet_live_rpc_simulation_receipt_result

The Stage 5.9 execution mode is:

    external_wallet_live_rpc_simulation_receipt_offline
...
The Stage 5.10 artifact is:

    stage5_external_wallet_live_submit_authorization_result

The Stage 5.10 execution mode is:

    external_wallet_live_submit_authorization_offline
...
The Stage 5.12 artifact is:

    stage5_external_wallet_live_confirmation_observation_result

The Stage 5.12 execution mode is:

    external_wallet_live_confirmation_observation_offline
...
External confirmation observation summary:
