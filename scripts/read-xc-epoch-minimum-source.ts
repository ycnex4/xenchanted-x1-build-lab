...
import {
  ...
  type EthereumScriptRunnerOutput,
...
class ConsoleOutput implements EthereumScriptRunnerOutput {
...
function createReadonlyPublicClient(config: {
  ...
  return {
    ...
    async getBlock(input) {
      if (input.blockNumber !== undefined) {
        return publicClient.getBlock({ blockNumber: input.blockNumber });
      }

      return publicClient.getBlock({ blockTag: input.blockTag ?? "latest" });
    },
...
function sanitizeErrorMessage(error: unknown): string {
  ...
  return "Manual RPC smoke script failed with a sanitized runtime error.";
}
...
async function main(): Promise<void> {
  ...
  const publicClient = createReadonlyPublicClient({
    chainId: config.chainId,
    rpcUrl: config.rpcUrl
  });

  const providerChainId = await publicClient.getChainId();
  ...
  console.log("manualRpcSmoke=true");
