...
#[test]
fn phase_41k6_b2_valid_quorum_live_gated_success_marks_processed_event_and_mints_spl() {
    ...
    let guardian_one_instruction =
        ed25519_precompile_instruction(fixture.payload_hash(), GUARDIAN_ONE);
    let guardian_two_instruction =
        ed25519_precompile_instruction(fixture.payload_hash(), GUARDIAN_TWO);
    let accounts = fixture.accounts_b1_v3();

    let expected_processed_event_data = build_final_consumed_processed_event_account_image(
        &CANONICAL_EVENT_KEY,
        &ROUTE_ID,
...
fn mollusk_for_program(program_id: &Pubkey) -> Mollusk {
    ...
    let token_program = TOKEN_PROGRAM_ID
        ...
        .expect("SPL Token program id");
    let spl_token_elf =
        find_litesvm_spl_token_elf().expect("LiteSVM SPL Token SBF ELF for B2 Mollusk CPI test");
    eprintln!("B2 using SPL Token ELF: {}", spl_token_elf.display());
    ...
    let ed25519_noop_elf =
        find_noop_aligned_sbf_elf().expect("noop SBF ELF for B2 prior Ed25519 harness stub");
    eprintln!(
        "B2 using Ed25519 noop stub ELF: {}",
...
fn ensure_sbf_b2_built() {
    SBF_BUILD.call_once(|| {
        ...
        assert!(
            status.success(),
            "cargo build-sbf B2 valid quorum live-gated success build failed"
        );
    });
}
