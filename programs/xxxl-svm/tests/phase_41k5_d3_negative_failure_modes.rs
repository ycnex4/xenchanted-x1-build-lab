...
#[test]
fn phase_41k5_d3_recipient_token_overflow_rolls_back_processed_event_mark() {
    ...
    let mut saturated_recipient_token_account =
        SplTokenAccount::unpack(&accounts[ACCOUNT_INDEX_RECIPIENT_TOKEN_ACCOUNT].1.data)
            .expect("valid recipient token account");
    saturated_recipient_token_account.amount = u64::MAX;

    let mut saturated_recipient_token_data = vec![0u8; SplTokenAccount::LEN];
    SplTokenAccount::pack(
        saturated_recipient_token_account,
        &mut saturated_recipient_token_data,
    )
    .expect("pack saturated recipient token account");

    accounts[ACCOUNT_INDEX_RECIPIENT_TOKEN_ACCOUNT].1.data = saturated_recipient_token_data.clone();

    let processed_event_before = accounts[ACCOUNT_INDEX_PROCESSED_EVENT].1.clone();
    let rent_payer_before = accounts[ACCOUNT_INDEX_RENT_PAYER].1.clone();
    let spl_mint_before = accounts[ACCOUNT_INDEX_SPL_MINT].1.clone();
    let recipient_token_before = accounts[ACCOUNT_INDEX_RECIPIENT_TOKEN_ACCOUNT].1.clone();
    let recipient_balance_before = accounts[ACCOUNT_INDEX_RECIPIENT_BALANCE].1.clone();
...
#[test]
fn phase_41k5_d3_already_consumed_processed_event_rejected_without_mint() {
    let fixture = ProductionPathFixture::new();
    let mollusk = mollusk_for_program(&fixture.program_id);
    ...
    let consumed_slot = mollusk.sysvars.clock.slot.saturating_add(7);
    let consumed_processed_event = build_final_consumed_processed_event_account_image(
        &CANONICAL_EVENT_KEY,
        &ROUTE_ID,
    ...
    .expect("consumed processed event image");
    ...
    accounts[ACCOUNT_INDEX_PROCESSED_EVENT].1 = account(
        rent_min,
        consumed_processed_event.as_ref().to_vec(),
        fixture.program_id,
        false,
    );
...
fn mollusk_for_program(program_id: &Pubkey) -> Mollusk {
    ...
    eprintln!("D3 using SPL Token ELF: {}", spl_token_elf.display());
...
fn ensure_sbf_d3_built() {
    SBF_BUILD.call_once(|| {
        let status = Command::new("cargo")
            ...
            .status()
            .expect("spawn cargo build-sbf for phase 41K.5 D3 negative failure modes");

        assert!(status.success(), "cargo build-sbf D3 negative failure mode build failed");
    });
}
